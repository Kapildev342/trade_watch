...
class CryptoEditFilterPage extends StatefulWidget {
  final String text;
  final String filterId;
  final String page;
  final dynamic response;

  const CryptoEditFilterPage({Key? key, required this.text, required this.filterId, required this.page, required this.response}) : super(key: key);

  @override
  State<CryptoEditFilterPage> createState() => _CryptoEditFilterPageState();
}

class _CryptoEditFilterPageState extends State<CryptoEditFilterPage> {
  ...
  getAllData() async {
    ...
    sendIndustriesList = response["response"]["industries"].toList();
    sendIndustriesListCheck = response["response"]["industries"].toList();
    setState(() {
      ...
      if (mounted) {
        newSetState(() {
          ...
          for (int i = 0; i < responseData["response"].length; i++) {
            ...
            if (sendIndustriesList[0] == responseData["response"][i]["_id"]) {
              _selectedExchange = i.toString();
              _typeController.text = industriesNameList[i];
