...
class CommodityEditFilterPage extends StatefulWidget {
  final String text;
  final String filterId;
  final String page;
  final dynamic response;

  const CommodityEditFilterPage({Key? key, required this.text, required this.filterId, required this.page, required this.response}) : super(key: key);

  @override
  State<CommodityEditFilterPage> createState() => _CommodityEditFilterPageState();
}

class _CommodityEditFilterPageState extends State<CommodityEditFilterPage> {
  ...
  getAllData() async {
    ...
    setState(() {
      ...
      _countryController.text = response["response"]["country"];
      sendTickersList = response["response"]["tickers"];
      selectedTypeValue = response["response"]["country"];
    ...
    for (int i = 0; i < countriesList.length; i++) {
      if (countriesList[i] == response["response"]["country"]) {
        setState(() {
          selectedIdItem = response["response"]["country"];
          selectedIdItemCheck = response["response"]["country"];
    ...
    if (sendTickersList.length > 1 || tickerSelectAll == true) {
      ...
      _commodityTickersController.text = "None";
    }
                                  ...
                                  onTap: () async {
                                    ...
                                    if (_listNameController.text.isEmpty) {
                                      ...
                                      var responseValue = await _filterFunction.getUpdateFilter(
                                          ...
                                          cid: mainCatIdList[2],
                                          exc: selectedIdItem == "" ? response["response"]["country"] : selectedIdItem,
                                          indus: [],
                                          tickers: sendTickersList.isEmpty ? response["response"]["tickers"] : sendTickersList,
                                          allIndus: false,
