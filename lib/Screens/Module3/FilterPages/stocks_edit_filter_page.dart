...
class StocksEditFilterPage extends StatefulWidget {
  final String text;
  final String filterId;
  final String page;
  final Map<String, dynamic> response;

  const StocksEditFilterPage({Key? key, required this.text, required this.filterId, required this.page, required this.response}) : super(key: key);

  @override
  State<StocksEditFilterPage> createState() => _StocksEditFilterPageState();
}

class _StocksEditFilterPageState extends State<StocksEditFilterPage> {
  ...
  getAllData() async {
    ...
    selectAllButton = response["response"]["allindustries"];
    ...
    editExchange = response["response"]["exchange"];
    sendIndustriesList = response["response"]["industries"];
    sendIndustriesListCheck = response["response"]["industries"];
    sendTickersList = response["response"]["tickers"];
    if (sendIndustriesList.length > 1 || selectAll == true) {
      _industriesController.text = "Multiple";
    } else if (sendIndustriesList.length == 1) {
      _industriesController.text = "Single";
    } else {
      _industriesController.text = "None";
    }
    if (sendTickersList.length > 1 || tickerSelectAll == true) {
      ...
      _stockTickersController.text = "None";
    }
    await getEx();
      ...
      if (mounted) {
        tickerSetState1(() {
          for (int i = 0; i < responseData["response"].length; i++) {
            ...
            /*if(sendIndustriesList.length>0){
              if(sendIndustriesList.contains(responseData["response"][i]["_id"])){
                tickerSetState1((){
                  indusBool.add(true);
                });
                                        ...
                                        onTap: () async {
                                          ...
                                          if (_listNameController.text.isEmpty) {
                                            ...
                                            var responseValue = await _filterFunction.getUpdateFilter(
                                                ...
                                                cid: mainCatIdList[0],
                                                exc: selectedIdItem == "" ? editExchange : selectedIdItem,
