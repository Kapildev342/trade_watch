...
class FlutterSummernoteState extends State<FlutterSummernote> {
  ...
  void initState() {
    ...
    _webViewController!.addJavaScriptChannel('GetTextSummernote', onMessageReceived: (JavaScriptMessage message) {
    ...
    _webViewController!.setNavigationDelegate(NavigationDelegate(onPageFinished: (String url) {
      if (widget.hint != null) {
        setHint(widget.hint);
      } else {
        setHint('');
    ...
    var toolbar = [
      Expanded(
        child: GestureDetector(
          ...
          child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[Icon(Icons.content_copy), Text('Copy')]),
        ),
      ),
      Expanded(
        child: GestureDetector(
      ...
      toolbar.insert(
          ...
          Expanded(
            child: GestureDetector(
              ...
              child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: <Widget>[Icon(Icons.attach_file), Text('Attach')]),
            ),
          ));
    }
  ...
  Future<void> setText(String v) async {
    String txtIsi = v
        ...
        .replaceAll('\r', ' ')
        .replaceAll('\r\n', ' ');
    String txt = 'document.getElementsByClassName(\'note-editable\')[0].innerHTML'
        ' = \'$txtIsi\';';
    _webViewController!.runJavaScript(txt);
  }
  ...
  void setFullContainer() {
    _webViewController!.runJavaScript('\$("#summernote").summernote("fullscreen.toggle");');
  }
...
""";
  ...
  Future<XFile?> _getImage(bool fromCamera) async {
    final picked = await _imagePicker.pickImage(source: (fromCamera) ? ImageSource.camera : ImageSource.gallery);
    if (picked != null) {
  ...
  void _addImage(XFile image) async {
    ...
    String base64Image = '<img width="${widget.widthImage}" src="data:image/png;base64, '
