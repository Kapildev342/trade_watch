...
import 'package:tradewatchfinal/Edited_Packages/editor/html_editor.dart' hide NavigationActionPolicy, UserScript, ContextMenu;
...
class _HtmlEditorWidgetMobileState extends State<HtmlEditorWidget> {
  ...
  void initState() {
    ...
    if (widget.htmlEditorOptions.filePath != null) {
      ...
      filePath = 'lib/Functions/editor/assets/summernote-no-plugins.html';
    } else {
      filePath = 'lib/Functions/editor/assets/summernote.html';
    }
    super.initState();
  }
  ...
  void resetHeight() async {
    if (mounted) {
      ...
      await widget.controller.editorController!.evaluateJavascript(
          source: "\$('div.note-editable').outerHeight(${widget.otherOptions.height - (toolbarKey.currentContext?.size?.height ?? 0)});");
    }
  }
          ...
          child: Column(
            children: [
              widget.htmlToolbarOptions.toolbarPosition == ToolbarPosition.aboveEditor
                  ? ToolbarWidget(
                      key: toolbarKey, controller: widget.controller, htmlToolbarOptions: widget.htmlToolbarOptions, callbacks: widget.callbacks)
                ...
                child: InAppWebView(
                  ...
                  shouldOverrideUrlLoading: (controller, action) async {
                    if (!action.request.url.toString().contains(filePath)) {
                      return (await widget.callbacks?.onNavigationRequestMobile?.call(action.request.url.toString())) as NavigationActionPolicy? ??
                  ...
                  onLoadStop: (InAppWebViewController controller, Uri? uri) async {
                    ...
                    if (url.contains(filePath)) {
                      ...
                      if ((Theme.of(context).brightness == Brightness.dark || widget.htmlEditorOptions.darkMode == true) &&
                        ...
                        var darkCSS =
                            '<link href=\"${(widget.htmlEditorOptions.filePath != null ? "file:///android_asset/flutter_assets/lib/Functions/editor/assets/" : "") + "summernote-lite-dark.css"}\" rel=\"stylesheet\">';
